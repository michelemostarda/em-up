...
/**
 * Defines the <i>number</i> terminal.
 */
public class MUNumeric extends DefaultTerminal  {

    public MUNumeric(String name) {
        super(name);
    }
    ...
    public TreeNode satisfied(Token token) throws ParserException {
        ...
        double doubleValue;
        try {
            doubleValue = Double.parseDouble( variable );
        } catch (NumberFormatException nfe) {
            throw new BacktrackingParserException("token value '" + variable + "' is not a valid const value.", this, token);
        }
        return new TreeNode(Double.toString(doubleValue), this);
    }

    @Override
    public Object postcompile(TreeNode associatedNode, Object[] childrenResults) {
        assert childrenResults.length == 0;
        return Double.parseDouble( associatedNode.getName() );
    }
