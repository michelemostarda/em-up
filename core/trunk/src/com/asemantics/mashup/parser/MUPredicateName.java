...
/**
 * Defines a <i>predicate_name</i> non terminal.
 */
public class MUPredicateName extends DefaultTerminal implements Compilable {
     ...
     * @param s
     */
    public MUPredicateName(String s) {
        super(s);
    }
    ...
    public TreeNode satisfied(Token token) throws ParserException {
        ...
        if(Character.isUpperCase(char0) && Util.isValidIdentifierStart( char0 ) ) {
            return new TreeNode(variable, this);
        }
        throw new BacktrackingParserException("token value '" + variable + "' is not a valid predicate name.", this, token);
    }
